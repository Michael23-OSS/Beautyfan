...
//select m.nombre as  barbero,p.nombre as  beneficiario,c.fecha,c.observaciones,c.estado_cita,c.id_cita from pedidos c inner join usuario m on c.id_barbero = m.id inner join usuario p on p.id = c.id_cliente
   // $branch=$_SESSION['branch'];
    $query=mysqli_query($con,"select m.nombre as  barbero,p.nombre as  beneficiario,c.id_pedido,c.fecha from pedidos c inner join usuario m on c.id_barbero = m.id inner join usuario p on p.id = c.id_cliente  ")or die(mysqli_error());
...
<a class="btn btn-primary btn-print" href="<?php  echo "../beneficiario/eliminar_cita.php?id_cita=$cid";?>"  onClick="return confirm('¿Está seguro de que quieres eliminar??');"  role="button">Eliminar</a>
      <?php
