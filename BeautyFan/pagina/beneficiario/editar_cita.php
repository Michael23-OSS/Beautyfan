...
<?php
   // $branch=$_SESSION['branch'];
    $query=mysqli_query($con,"select m.nombre as  barbero,p.nombre as  beneficiario,c.fecha,c.observaciones,c.estado_cita,c.id_cita from cita c inner join usuario m on c.id_barbero = m.id inner join usuario p on p.id = c.id_beneficiario
 where c.id_cita='$id_cita' ")or die(mysqli_error());
    $i=1;
    while($row=mysqli_fetch_array($query)){
$id_medico=$row['barbero'];
